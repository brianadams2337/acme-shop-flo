...
import CountryDetection from './CountryDetection.vue'
...
describe('countryDetection.vue', () => {
  beforeEach(() => {
    suggestionActive.value = true
  })
  it('should autofocus first suggested shop', async () => {
    const { getByTestId } = await renderSuspended(CountryDetection)
  ...
  it('should close dialog and emit event when suggested shop was chosen', async () => {
    const { getByTestId, emitted } = await renderSuspended(CountryDetection)
    const dialog = getByTestId('country-detection-dialog')
    expect(dialog).toBeVisible()
