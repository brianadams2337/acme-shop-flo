...
_Based on <https://github.com/zadigetvoltaire/nuxt-gtm>_

## Quick Setup

1. As `@scayle/nuxt-gtm` is as local module, it does not need to be installed manually.
...
2. Add configuration in `nuxtConfig.gtm` or in `nuxtConfig.runtimeConfig.public.gtm`
...
<template>
  ...
  <button @click="triggerView">Trigger event!</button>
</template>
